...
Using the pairwise transformation parameters determined previously, the next step is to compute global transformation parameters (e.g. transformations for each image in cube relative to one image). The algorithm is as follows:
	...
	Let the global transformation parameters for an $mxn$ image $i$ be $T_{global}(i,:)$ and for $T_{global,m,n}$ = $0_{m,n}$ initially, with $T_{global}(1,:)$ = $[0, 0, 0]$ always true. \\ 
	...
	6) Repeat Steps 1-5 for remaining pairwise, unaligned images to generate $I_{aligned}$. 
	\end{algorithm}
